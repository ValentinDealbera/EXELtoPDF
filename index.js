...
const dataToPrint = formatData.map((e, i) => {
  return e.filter((element, index) => element !== null);
});
const printData = dataToPrint.map((e, i) => {
  const headers = dataToPrint[0];
  const obj = {};
...
const dataForPrinting = printData.filter((_, index) => index >= 1);

res.status(200).json(dataForPrinting);
  } catch (error) {
